import type { LandingFeature, LandingInfoCard } from './types'

export const homeMeta = {
  title: 'XK HARDCORE | Private Minecraft RolePlay Server',
  description:
    'XK HARDCORE — приватный Minecraft RolePlay сервер в духе Dream SMP: ваниль, RP-жизни, города, королевства и живой мир, где важны союзники, характер и история.',
}

export const heroContent = {
  caption: 'Приватный майнкрафт сервер',
  title: 'XK HARDCORE',
  subtitle: 'РП • Ванилла • Живой мир',
  description:
    'Сервер, который не является однодневкой и не закроется из-за неуплаты хостинга. Ванильный мир с RP-жизнями, городами, королевствами и решениями, которые реально меняют историю.',
}

export const reasons: LandingInfoCard[] = [
  {
    title: 'Города и королевства',
    text: 'Строй государства, заключай союзы и веди дипломатию, которая меняет карту мира.',
    tone: 'green',
  },
  {
    title: 'Жизни',
    text: 'У каждого игрока всего 2 жизни, и каждая из них делает историю ценнее. Если всё пошло не по плану, другого игрока можно успеть поднять в течение нескольких минут после смерти. Когда жизни заканчиваются, их можно купить на сайте. Здесь особенно важно держаться вместе и не теряться с другими игроками.',
    tone: 'violet',
  },
  {
    title: 'Стабильность TPS',
    text: 'Сервер работает на мощном оборудовании и выдерживает высокую нагрузку без лагов. Команда постоянно оптимизирует сервер и плагины, чтобы ваш пинг не улетал на Марс.',
    tone: 'gold',
  },
]

export const features: LandingFeature[] = [
  {
    title: 'Dream SMP вайб',
    text: 'Сервер вдохновлён форматом сюжетного SMP, где самое важное создают сами игроки.',
    tone: 'violet',
  },
  {
    title: 'Защита аккаунта',
    text: 'Можно привязать аккаунт к Telegram или играть через лицензионный аккаунт, чтобы вход был надёжным и удобным.',
    tone: 'blue',
  },
]

export const applyContent = {
  badge: 'Анкета на вступление',
  title: 'Подавайте заявку',
  mark: 'И присоединяйтесь к XK HARDCORE',
  telegramLabel: 'Открыть анкету',
  buyPassLabel: 'Купить проходку',
}
